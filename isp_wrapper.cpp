...
int isp_cmd_compare(int arg_count, char **args)
{
	...
	ret = isp_cmd_address_wrapper(arg_count, args, (char *)"compare", 'M');
	switch (ret)
	{
	case CMD_SUCCESS:
...
int isp_cmd_copy_ram_to_flash(int arg_count, char **args)
{
	...
	ret = isp_cmd_address_wrapper(arg_count, args, (char *)"copy-ram-to-flash", 'C');
...
int isp_cmd_blank_check(int arg_count, char **args)
{
	...
	ret = isp_cmd_sectors_skel(arg_count, args, (char *)"blank-check", 'I');
	if (ret < 0)
	{
		return ret;
	}
...
{
	int ret = isp_cmd_sectors_skel(arg_count, args, (char *)"prepare-for-write", 'P');
...
{
	int ret = isp_cmd_sectors_skel(arg_count, args, (char *)"erase", 'E');
