...
int isp_serial_write(const char *buf, unsigned int buf_size)
{
	...
	do
	{
		int err = serial.writeBytes(buf + count, buf_size - count, &nb);
...
void isp_serial_empty_buffer()
{
	...
	do
	{
		...
		nb = serial.readBytes(&unused, 1, 250, 500);
		if (nb < 0)
		{
			printf("Serial read error: serialib returned %d\n", nb);
			return;
		}
		else if (nb == 0)
		{
			printf("serial_read: timeout, no data received.\n");
			break;
		}
