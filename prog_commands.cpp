...
int erase_flash(struct part_desc *part)
{
	...
	{
		ret = isp_send_cmd_sectors((char *)"blank-check", 'I', i, i, 1);
		if (ret == CMD_SUCCESS)
		{
		...
		ret = isp_send_cmd_sectors((char *)"prepare-for-write", 'P', i, i, 1);
		if (ret != 0)
		{
			printf("Error (%d) when trying to prepare sector %d for erase operation!\n", ret, i);
			return ret;
		}
		ret = isp_send_cmd_sectors((char *)"erase", 'E', i, i, 1);
		if (ret != 0)
		{
...
int flash_target(struct part_desc *part, char *filename, int calc_user_code)
{
	...
	for (i = 0; i < blocks; i++)
	{
		...
		ret = isp_send_cmd_sectors((char *)"prepare-for-write", 'P', current_sector, current_sector, 1);
		if (ret != 0)
		{
			printf("Error (%d) when trying to prepare sector %d for erase operation!\n", ret, i);
			free(data);
...
int non_uniform_flash_target(struct part_desc *part, char *filename, int calc_user_code)
{
	...
	int i = 0, blocks = 0;
	unsigned int write_size = 0;
	// set size based on initial section section size
	...
	/* And fill the buffer with the image */
	image_size = isp_file_to_buff(data, part->flash_size, filename);
	if (image_size <= 0)
	{
		free(data);
		return -5;
	...
	/* Fill unused buffer with 0's so we can flash blocks of data of "write_size" */
	memset(&data[image_size], 0, (part->flash_size - image_size));
	/* And check checksum of first 7 vectors if asked, according to section 21.3.3 of
	 * LPC11xx user's manual (UM10398) */
	v = (uint32_t *)data;
	cksum = 0 - v[0] - v[1] - v[2] - v[3] - v[4] - v[5] - v[6];
	...
	// set first blocks
	blocks = (0x10000 / write_size) + ((0x10000 % write_size) ? 1 : 0);
	/* Gonna write out of flash ? */
	if ((blocks * write_size) > part->flash_size)
	{
		printf("Config error, I cannot flash beyond end of flash !\n");
	...
	for (i = 0; i < blocks; i++)
	{
		...
		/* Send data to RAM */
		ret = isp_send_buf_to_ram(&data[i * write_size], ram_addr, write_size, uuencode);
		last_data_index = (i * write_size) + write_size;
		printf("writing data: %d - %d\n", i * write_size, last_data_index);
	...
	// set first blocks
	blocks = (0x70000 / write_size) + ((0x70000 % write_size) ? 1 : 0);
	/* Gonna write out of flash ? */
	if ((blocks * write_size) > part->flash_size)
	{
		printf("Config error, I cannot flash beyond end of flash !\n");
	...
	for (i = 0; i < blocks; i++)
	{
		...
		/* Send data to RAM */
		ret = isp_send_buf_to_ram(&data[(i * write_size) + last_data_index], ram_addr, write_size, uuencode);
		printf("writing data: %d - %d\n", (i * write_size) + last_data_index, (i * write_size) + last_data_index + write_size);
